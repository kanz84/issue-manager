...
class TaskViewSetTestCase(TestCase):
    def setUp(self):
        ...
        result = self.client.login(password='test', username='test', )
    ...
    def test_get_list(self):
        ...
        self.assertEqual(
            set(task['id'] for task in response.data),
    ...
    def test_create(self):
        ...
        user = UserFactory(username="test_su", password="test_su", email='test_suuser@example.com', is_staff=True,
                           is_superuser=True)
        user.set_password('test_su')
        user.save()

        result = self.client.login(password='test_su', username='test_su', )
        logger.info('login result is :%s', result)
